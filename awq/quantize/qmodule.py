...
# There is a lot of duplicated code here, but this will be the fastest way to test it out
# We can extend this to a WQTriton as well when we have the triton kernels in
# Again most of the code is duplicated but its not such a big deal for now


class WQLinearGEMV(nn.Module):
    """The Linear Class that uses the GEMV implementation of AWQ."""
...
WQLinear = WQLinearGEMV if USE_GEMV else WQLinearGEMM
